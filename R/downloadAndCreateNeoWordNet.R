...
#' Download WordNet files and create a WordNet graph database in Neo4j
#'
#' \code{downloadAndCreateNeoWordNet} downloads and processes WordNet index and
...
#' @param dictPath A path to the WordNet data and index files. Defaults to
#'   working directory.
...
downloadAndCreateNeoWordNet <- function(wordNetUrl = "http://wordnetcode.princeton.edu/wn3.1.dict.tar.gz",
  ...
  runIntegrationTests()
  dictPath <- downloadWordNetFiles(wordNetUrl = wordNetUrl, dest = dest, verbose = verbose)
  createWordNetGraph(dictPath = dictPath, verbose = verbose, url = url, username=username, password=password)
}
